...
def run_config_smoke():
    ...
    try:
        ...
        try:
            algo = config.build()
            print("[+] Algorithm built successfully. Running 3 iteration steps for stability...")
            
            for i in range(1, 4):
                results = algo.train()
                reward_mean = results.get("env_runners", {}).get("episode_reward_mean", results.get("episode_reward_mean", "N/A"))
                print(f"    - Iteration {i} | Reward mean: {reward_mean}")
                
            print(f"[+] SUCCESS: 3 PPO iterations completed cleanly.")
            algo.stop()
